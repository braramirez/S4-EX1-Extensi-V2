CLASS lsc_zbp_i_online_shop_c04b DEFINITION INHERITING FROM cl_abap_behavior_saver.
  PROTECTED SECTION.
    METHODS save_modified REDEFINITION.
ENDCLASS.

CLASS lsc_zbp_i_online_shop_c04b IMPLEMENTATION.

  METHOD save_modified.
    DATA : lt_online_shop_as TYPE STANDARD TABLE OF zshop_as_c04b,
           ls_online_shop_as TYPE                   zshop_as_c04b.
    IF zbp_i_online_shop_c04b=>cv_pr_mapped-purchaserequisition IS NOT INITIAL.
      LOOP AT zbp_i_online_shop_c04b=>cv_pr_mapped-purchaserequisition ASSIGNING FIELD-SYMBOL(<fs_pr_mapped>).
    ...
    IF create-online_shop IS NOT INITIAL.
      ...
      INSERT zshop_as_c04b FROM TABLE @lt_online_shop_as.
    ENDIF.
  ENDMETHOD.
ENDCLASS.

CLASS lhc_zbp_i_online_shop_c04b  DEFINITION INHERITING FROM cl_abap_behavior_handler.
  PRIVATE SECTION.
...
CLASS lhc_zbp_i_online_shop_c04b  IMPLEMENTATION.
...
*  if a new package is ordered, trigger a new purchase requisition
      ...
      zbp_i_online_shop_c04b=>cv_pr_mapped = ls_pr_mapped.
    ENDLOOP.
...
* Update an existing info record
  ...
  METHOD calculate_order_id.
    DATA:
      online_shops TYPE TABLE FOR UPDATE zi_online_shop_c04b,
      online_shop  TYPE STRUCTURE FOR UPDATE zi_online_shop_c04b.

    SELECT MAX( order_id ) FROM zonlineshop_c04b INTO @DATA(max_order_id).

    READ ENTITIES OF zi_online_shop_c04b IN LOCAL MODE
       ENTITY Online_Shop
        ALL FIELDS
    ...
    MODIFY ENTITIES OF zi_online_shop_c04b IN LOCAL MODE
    ENTITY Online_Shop UPDATE SET FIELDS WITH online_shops
    MAPPED   DATA(ls_mapped_modify)
    FAILED   DATA(lt_failed_modify)
    REPORTED DATA(lt_reported_modify).

    DATA: lt_create_pr_imp TYPE TABLE FOR ACTION IMPORT  zi_online_shop_c04b~create_pr,
